...
./getDockerImageAndContainer.386 -serverIp=${ServerIp} -getIp=true -ipListFile=${IpListFile}
...
do
    ...
    if [ ${Ip}x == ${ip2}x ]; then
        ...
        ret1=`./sshrpc.386 -ip=${Ip} -p=false -l="ps -A -opid,stime,etime,args | grep reload_sensor.sh"`
        ...
        # 测试设备启动脚本是否损坏
        re4=`./sshrpc.386 -ip=${Ip} -p=false -l="ls /usr/bin/tegra_init.sh -l |cut -d \" \" -f 5"`
        re5=`./sshrpc.386 -ip=${Ip} -p=false -l="find /usr/bin/ -name tegra_init.sh"`
        # echo "/usr/bin/tegra_init.sh文件的大小为:${re4}"
        if [[ (-z ${re5} ) || (${re4} -lt 1754) ]];then
            echo "${Ip},设备的启动脚本损坏，请处理！"
        fi
        # 获取设备上正在运行的container和images以及设备版本
        ./getDockerImageAndContainer.386 -sensorIp=${Ip} -getDocker=true
    else
        echo "${Ip} ping 不通,请检查!"
    fi
done
