...
[ -d "/data/eventserver" ] || sudo mkdir /data/eventserver
...
PIONEER_IMAGE="192.168.5.46:5000/armhf-pioneer:1.5.4_hf"
...
do
    echo $IMAGE
    if [ \( "$IMAGE"x == "/config/image/armhf-pioneer"x \) -o \( "$IMAGE"x == "/config/image/armhf-libra-init"x \) -o \( "$IMAGE"x == "/config/image/armhf-etcd"x \) -o \( "$IMAGE"x == "/config/image/armhf-nsqd-live"x \) -o \( "$IMAGE"x == "/config/image/armhf-crtmpserver"x \) -o \( "$IMAGE"x == "/config/image/armhf-vodserver"x \) ]; then
    	#statements
    	echo "Not now."
    	continue
    fi
